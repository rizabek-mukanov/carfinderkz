...
```json
{ "telegram": { "ok": true, "photo": true, "text": true } }
```

В чат уходят **два** сообщения:

1. **PNG-инфографика** (сводка + список цен со стрелками) + короткий caption  
2. **HTML-текст** — полный отчёт

Если PNG не собрался, текст всё равно отправится (`photo: false, text: true`).

## 5. Формат отчёта

### Картинка (`src/lib/price-infographic.tsx`)

- Заголовок, дата (Алматы), среднее изменение  
- Чипы: выросли / упали / без изменений / новые  
- Строки по моделям: цена, дельта, «было»  

### Текст (`src/lib/price-report.ts`)
